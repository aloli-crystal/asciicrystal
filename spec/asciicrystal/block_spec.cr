...
def convert_string_docbook(input : String) : String
  Asciicrystal.convert(input, {"backend" => "docbook5"})
end
...
describe "Blocks" do
  ...
  describe "Passthrough Blocks" do
    it "can parse a passthrough block" do
      ...
      block = doc.blocks[0].as(Asciicrystal::Block)
  ...
  describe "#initialize" do
    it "creates a block with default content model for paragraph" do
      ...
      block.content_model.should eq(Asciicrystal::ContentModel::Simple)
    ...
    it "creates a block with default content model for listing" do
      ...
      block.content_model.should eq(Asciicrystal::ContentModel::Verbatim)
    end

    it "creates a block with default content model for image" do
      doc = Asciicrystal::Document.new
      block = Asciicrystal::Block.new(doc, :image)
      block.content_model.should eq(Asciicrystal::ContentModel::Empty)
    end

    it "creates a block with source string" do
      doc = Asciicrystal::Document.new
      block = Asciicrystal::Block.new(doc, :paragraph, source: "Hello World")
      block.lines.should eq(["Hello World"])
    end

    it "creates a block with source array" do
      doc = Asciicrystal::Document.new
      block = Asciicrystal::Block.new(doc, :paragraph, source: ["line 1", "line 2"])
      block.lines.should eq(["line 1", "line 2"])
    end

    it "creates a block with custom content model" do
      doc = Asciicrystal::Document.new
      block = Asciicrystal::Block.new(doc, :open, content_model: Asciicrystal::ContentModel::Simple)
      block.content_model.should eq(Asciicrystal::ContentModel::Simple)
    end
  end
  ...
  describe "#content" do
    ...
    it "strips leading and trailing blank lines for verbatim content" do
      doc = Asciicrystal::Document.new
      block = Asciicrystal::Block.new(doc, :listing, source: ["", "code here", "more code", ""])
