...
describe Asciicrystal::Parser do
  describe ".adjust_indentation!" do
    it "removes common leading whitespace" do
      lines = ["  line 1", "  line 2", "  line 3"]
      Asciicrystal::Parser.adjust_indentation!(lines)
  ...
  describe ".atx_section_title?" do
    it "detects level 0 section title" do
      Asciicrystal::Parser.atx_section_title?("= Title").should eq(0)
    end

    it "detects level 1 section title" do
      Asciicrystal::Parser.atx_section_title?("== Section").should eq(1)
    end

    it "detects level 2 section title" do
      Asciicrystal::Parser.atx_section_title?("=== Subsection").should eq(2)
    end

    it "detects level 3 section title" do
      Asciicrystal::Parser.atx_section_title?("==== Level 3").should eq(3)
    end

    it "detects level 4 section title" do
      Asciicrystal::Parser.atx_section_title?("===== Level 4").should eq(4)
    end

    it "returns nil for non-section lines" do
      Asciicrystal::Parser.atx_section_title?("just a line").should be_nil
    end

    it "returns nil for line starting with = but no space" do
      Asciicrystal::Parser.atx_section_title?("=nospace").should be_nil
    end
  ...
  describe ".is_delimited_block?" do
    it "detects listing block" do
      result = Asciicrystal::Parser.is_delimited_block?("----", true)
    ...
    it "returns nil for non-delimited lines" do
      Asciicrystal::Parser.is_delimited_block?("just a line", true).should be_nil
    end

    it "returns nil for short lines" do
      Asciicrystal::Parser.is_delimited_block?("-", true).should be_nil
    end
  ...
  describe ".is_section_title?" do
    it "detects atx section title" do
      Asciicrystal::Parser.is_section_title?("== Title").should eq(1)
    end

    it "returns nil for non-section lines" do
      Asciicrystal::Parser.is_section_title?("just a line").should be_nil
    end
  end
  ...
  describe ".parse_block_attribute_list" do
    it "parses a simple style" do
      ...
      Asciicrystal::Parser.parse_block_attribute_list("source", attrs)
      attrs["1"].should eq("source")
    end

    it "parses key=value pairs" do
      attrs = {} of String => String
      Asciicrystal::Parser.parse_block_attribute_list("source,ruby", attrs)
      attrs["1"].should eq("source")
    ...
    it "parses named attributes" do
      attrs = {} of String => String
      Asciicrystal::Parser.parse_block_attribute_list("role=lead", attrs)
      attrs["role"].should eq("lead")
    end

    it "parses shorthand id" do
      attrs = {} of String => String
      Asciicrystal::Parser.parse_block_attribute_list("source#myid", attrs)
    ...
    it "parses shorthand role" do
      attrs = {} of String => String
      Asciicrystal::Parser.parse_block_attribute_list("source.myrole", attrs)
      attrs["style"].should eq("source")
    ...
    it "parses shorthand option" do
      attrs = {} of String => String
      Asciicrystal::Parser.parse_block_attribute_list("source%nowrap", attrs)
      attrs["style"].should eq("source")
  ...
  describe ".parse_block_metadata_line" do
    it "parses a block anchor" do
      reader = Asciicrystal::Reader.new(["[[myid]]", "content"])
      doc = Asciicrystal::Document.new
      attrs = {} of String => String
      result = Asciicrystal::Parser.parse_block_metadata_line(reader, doc, attrs)
      result.should be_true
      attrs["id"].should eq("myid")
    end

    it "parses a block title" do
      reader = Asciicrystal::Reader.new([".My Title", "content"])
      doc = Asciicrystal::Document.new
      attrs = {} of String => String
      result = Asciicrystal::Parser.parse_block_metadata_line(reader, doc, attrs)
      result.should be_true
    ...
    it "skips single-line comments" do
      reader = Asciicrystal::Reader.new(["// comment", "content"])
      doc = Asciicrystal::Document.new
      attrs = {} of String => String
      result = Asciicrystal::Parser.parse_block_metadata_line(reader, doc, attrs)
      result.should be_true
    ...
    it "returns false for regular content" do
      reader = Asciicrystal::Reader.new(["just content"])
      doc = Asciicrystal::Document.new
      attrs = {} of String => String
      result = Asciicrystal::Parser.parse_block_metadata_line(reader, doc, attrs)
  ...
  describe ".parse_section_title" do
    it "parses an atx section title" do
      reader = Asciicrystal::Reader.new(["== My Section", "content"])
      doc = Asciicrystal::Document.new
      id, reftext, title, level, atx = Asciicrystal::Parser.parse_section_title(reader, doc)
    ...
    it "parses a level 0 title" do
      reader = Asciicrystal::Reader.new(["= Document Title", "content"])
      doc = Asciicrystal::Document.new
      id, reftext, title, level, atx = Asciicrystal::Parser.parse_section_title(reader, doc)
    ...
    it "parses a level 3 title" do
      reader = Asciicrystal::Reader.new(["==== Deep Section", "content"])
      doc = Asciicrystal::Document.new
      id, reftext, title, level, atx = Asciicrystal::Parser.parse_section_title(reader, doc)
  ...
  describe ".process_authors" do
    it "parses a single author" do
      metadata = Asciicrystal::Parser.process_authors("John Doe")
    ...
    it "parses an author with middle name" do
      metadata = Asciicrystal::Parser.process_authors("John Michael Doe")
    ...
    it "parses a single-name author" do
      metadata = Asciicrystal::Parser.process_authors("John")
    ...
    it "parses multiple authors separated by semicolons" do
      metadata = Asciicrystal::Parser.process_authors("John Doe; Jane Smith")
    ...
    it "handles underscores in names" do
      metadata = Asciicrystal::Parser.process_authors("John_Michael Doe")
  ...
  describe ".resolve_ordered_list_marker" do
    it "resolves dot markers" do
      marker, style = Asciicrystal::Parser.resolve_ordered_list_marker(".")
      marker.should eq(".")
      style.should be_nil
    end

    it "resolves numeric markers" do
      marker, style = Asciicrystal::Parser.resolve_ordered_list_marker("1.")
      marker.should eq("1.")
      style.should eq(:arabic)
    end

    it "resolves lowercase alpha markers" do
      marker, style = Asciicrystal::Parser.resolve_ordered_list_marker("a.")
      marker.should eq("a.")
    ...
    it "normalises any arabic marker to `1.` regardless of input" do
      ...
      marker, _ = Asciicrystal::Parser.resolve_ordered_list_marker("2.", nil, false)
      marker.should eq("1.")

      marker, _ = Asciicrystal::Parser.resolve_ordered_list_marker("42.", nil, false)
      marker.should eq("1.")
    end
  end
  ...
  describe ".sanitize_attribute_name" do
    it "lowercases the name" do
      Asciicrystal::Parser.sanitize_attribute_name("MyAttr").should eq("myattr")
    end

    it "removes invalid characters" do
      Asciicrystal::Parser.sanitize_attribute_name("my attr!").should eq("myattr")
    end
  end
  ...
  describe ".store_attribute" do
    it "stores an attribute in the document" do
      doc = Asciicrystal::Document.new
      Asciicrystal::Parser.store_attribute("myattr", "myvalue", doc)
    ...
    it "unsets an attribute with trailing bang" do
      ...
      Asciicrystal::Parser.store_attribute("myattr!", "", doc)
    ...
    it "resolves numbered to sectnums" do
      doc = Asciicrystal::Document.new
      Asciicrystal::Parser.store_attribute("numbered", "", doc)
    ...
    it "handles relative leveloffset with +" do
      doc = Asciicrystal::Document.new
      doc.attributes["leveloffset"] = "1"
      Asciicrystal::Parser.store_attribute("leveloffset", "+2", doc)
    ...
    it "handles relative leveloffset with -" do
      ...
      Asciicrystal::Parser.store_attribute("leveloffset", "-1", doc)
  ...
  describe ".uniform?" do
    it "returns true for uniform string" do
      Asciicrystal::Parser.uniform?("====", "=", 4).should be_true
    end

    it "returns false for non-uniform string" do
      Asciicrystal::Parser.uniform?("==-=", "=", 4).should be_false
    end

    it "returns false for empty string" do
      Asciicrystal::Parser.uniform?("", "=", 0).should be_false
    end
  end

  describe "next_block" do
    it "parses a simple paragraph" do
      reader = Asciicrystal::Reader.new(["This is a paragraph.", "With two lines."])
      doc = Asciicrystal::Document.new
      block = Asciicrystal::Parser.next_block(reader, doc)
      block.should_not be_nil
      block.not_nil!.context.should eq(:paragraph)
    end

    it "parses a listing block" do
      reader = Asciicrystal::Reader.new(["----", "code here", "----"])
      doc = Asciicrystal::Document.new
      block = Asciicrystal::Parser.next_block(reader, doc)
      block.should_not be_nil
    ...
    it "parses a sidebar block" do
      reader = Asciicrystal::Reader.new(["****", "sidebar content", "****"])
      doc = Asciicrystal::Document.new
      block = Asciicrystal::Parser.next_block(reader, doc)
      block.should_not be_nil
    ...
    it "parses a literal block" do
      reader = Asciicrystal::Reader.new(["....", "literal content", "...."])
      doc = Asciicrystal::Document.new
      block = Asciicrystal::Parser.next_block(reader, doc)
      block.should_not be_nil
    ...
    it "parses a thematic break" do
      reader = Asciicrystal::Reader.new(["'''", "next content"])
      doc = Asciicrystal::Document.new
      block = Asciicrystal::Parser.next_block(reader, doc)
      block.should_not be_nil
    ...
    it "parses an indented paragraph as literal" do
      reader = Asciicrystal::Reader.new(["  indented line 1", "  indented line 2"])
      doc = Asciicrystal::Document.new
      block = Asciicrystal::Parser.next_block(reader, doc)
      block.should_not be_nil
      block.not_nil!.context.should eq(:literal)
    ...
    it "returns nil when reader is empty" do
      reader = Asciicrystal::Reader.new([] of String)
      doc = Asciicrystal::Document.new
      block = Asciicrystal::Parser.next_block(reader, doc)
      block.should be_nil
    end
  ...
  describe "initialize_section" do
    it "creates a section from atx title" do
      reader = Asciicrystal::Reader.new(["== My Section", "", "content"])
      doc = Asciicrystal::Document.new
      section = Asciicrystal::Parser.initialize_section(reader, doc)
    ...
    it "creates a chapter for book doctype level 1" do
      reader = Asciicrystal::Reader.new(["== Chapter One", "", "content"])
      doc = Asciicrystal::Document.new(doctype: "book")
      section = Asciicrystal::Parser.initialize_section(reader, doc)
      section.sectname.should eq("chapter")
    end

    it "creates a part for book doctype level 0" do
      reader = Asciicrystal::Reader.new(["= Part One", "", "content"])
      doc = Asciicrystal::Document.new(doctype: "book")
      section = Asciicrystal::Parser.initialize_section(reader, doc)
...
describe Asciicrystal::Parser do
  describe ".catalog_callouts" do
    it "detects callout markers in source text" do
      doc = Asciicrystal::Document.new
      text = "puts 'hello' <1>\nputs 'world' <2>"
      result = Asciicrystal::Parser.catalog_callouts(text, doc)
      result.should be_true
    end
  ...
  describe ".catalog_inline_anchors" do
    it "catalogs inline anchors in text" do
      ...
      reader = Asciicrystal::Reader.new(["content"])
      text = "Some text [[myanchor]] more text"
      # Should not raise
      Asciicrystal::Parser.catalog_inline_anchors(text, block, doc, reader)
    end
  ...
  describe ".parse_cellspec" do
    it "parses text without cellspec at end position" do
      attrs, text = Asciicrystal::Parser.parse_cellspec("just text")
      attrs.should_not be_nil
      text.should eq("just text")
    end

    it "parses cellspec with horizontal alignment at start" do
      attrs, text = Asciicrystal::Parser.parse_cellspec(">|cell content", pos: :start, delimiter: "|")
      attrs.should_not be_nil
      if attrs
    ...
    it "parses cellspec with span at start" do
      attrs, text = Asciicrystal::Parser.parse_cellspec("2+|cell content", pos: :start, delimiter: "|")
      attrs.should_not be_nil
      if attrs
    ...
    it "returns nil attrs when no delimiter found at start" do
      attrs, text = Asciicrystal::Parser.parse_cellspec("no delimiter", pos: :start, delimiter: "|")
      attrs.should be_nil
  ...
  describe ".parse_callout_list" do
    it "parses a callout list" do
      ...
      callouts.register(1)
      reader = Asciicrystal::Reader.new(["<1> First callout", "<2> Second callout"])
      line = reader.read_line.not_nil!
      match = Asciicrystal::CalloutListRx.match(line)
      if match
        list = Asciicrystal::Parser.parse_callout_list(reader, match, doc, callouts)
  ...
  describe ".parse_list_item" do
    it "parses a simple unordered list item" do
      doc = Asciicrystal::Document.new
      list = Asciicrystal::List.new(doc, :ulist)
      reader = Asciicrystal::Reader.new(["* Item 1", "* Item 2"])
      line = reader.read_line.not_nil!
      match = Asciicrystal::UnorderedListRx.match(line)
      if match
        item = Asciicrystal::Parser.parse_list_item(reader, list, match, "*")
    ...
    it "parses a list item with continuation" do
      doc = Asciicrystal::Document.new
      list = Asciicrystal::List.new(doc, :ulist)
      reader = Asciicrystal::Reader.new(["* Item 1", "+", "Continuation text", "* Item 2"])
      line = reader.read_line.not_nil!
      match = Asciicrystal::UnorderedListRx.match(line)
      if match
        item = Asciicrystal::Parser.parse_list_item(reader, list, match, "*")
  ...
  describe ".resolve_ordered_list_start" do
    it "returns 1 for dot markers" do
      Asciicrystal::Parser.resolve_ordered_list_start(".").should eq(1)
    end

    it "returns the number for arabic markers" do
      Asciicrystal::Parser.resolve_ordered_list_start("3.").should eq(3)
    end

    it "returns the position for lowercase alpha markers" do
      Asciicrystal::Parser.resolve_ordered_list_start("c.").should eq(3)
    end
    ...
    it "returns the value for lowercase roman markers" do
      Asciicrystal::Parser.resolve_ordered_list_start("iii)").should eq(3)
    end
  ...
  describe ".resolve_ordered_list_marker (extended)" do
    it "resolves lowercase roman markers" do
      marker, style = Asciicrystal::Parser.resolve_ordered_list_marker("i)")
      marker.should eq("i)")
    ...
    it "validates ordinal when requested" do
      marker, style = Asciicrystal::Parser.resolve_ordered_list_marker("2.", ordinal: 1, validate: true)
      marker.should eq("1.")
      style.should eq(:arabic)
    end
  end
  ...
  describe ".yield_buffered_attribute" do
    it "stores a style attribute" do
      ...
      Asciicrystal::Parser.yield_buffered_attribute(attrs, nil, "source")
      attrs[:style].should eq("source")
    end

    it "stores an id attribute" do
      attrs = {} of Symbol => String | Array(String)
      Asciicrystal::Parser.yield_buffered_attribute(attrs, :id, "myid")
      attrs[:id].should eq("myid")
    end

    it "stores role attributes as array" do
      attrs = {} of Symbol => String | Array(String)
      Asciicrystal::Parser.yield_buffered_attribute(attrs, :role, "lead")
      Asciicrystal::Parser.yield_buffered_attribute(attrs, :role, "center")
      roles = attrs[:role]
    ...
    it "stores option attributes as array" do
      attrs = {} of Symbol => String | Array(String)
      Asciicrystal::Parser.yield_buffered_attribute(attrs, :option, "nowrap")
      Asciicrystal::Parser.yield_buffered_attribute(attrs, :option, "linenums")
    ...
    it "does not store empty values" do
      attrs = {} of Symbol => String | Array(String)
      Asciicrystal::Parser.yield_buffered_attribute(attrs, :id, "")
  ...
  describe ".read_lines_for_list_item" do
    it "reads lines for a simple list item" do
      reader = Asciicrystal::Reader.new(["line 1", "* next item"])
      lines = Asciicrystal::Parser.read_lines_for_list_item(reader, :ulist, "*")
      lines.should eq(["line 1"])
    end

    it "reads lines until blank line for list item" do
      reader = Asciicrystal::Reader.new(["line 1", "line 2", "", "next paragraph"])
      lines = Asciicrystal::Parser.read_lines_for_list_item(reader, :ulist, "*")
      lines.should eq(["line 1", "line 2"])
    end
  end
  ...
  describe "parse_manpage_header" do
    it "does not raise on empty document" do
      doc = Asciicrystal::Document.new(doctype: "manpage")
      reader = Asciicrystal::Reader.new([] of String)
      attrs = {} of String => String
      # Should not raise
